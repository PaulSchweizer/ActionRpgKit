...
public class SkillCreator : EditorWindow
{
    ...
    string[] _skillTypes = new string[] { "PassiveMagicSkill", "GenericCombatSkill", "OffensiveMagicSkill"};
    ...
    void CreateNewSkill()
    {
        ...
        {
            var skill = CreateOffensiveMagicSkill();
            AssetDatabase.CreateAsset(skill, Path.Combine(RelativePath,
                                      string.Format("MagicSkill/{0}_{1}.asset",
                                                    skill.Skill.Id,
                                                    _name)));
    ...
    /// <summary>
    /// Create an Offensive Magic Skill.</summary>
    /// <returns>A ScriptableObject</returns>
    OffensiveMagicSkillData CreateOffensiveMagicSkill()
    {
        var skill = new OffensiveMagicSkill(
                            id: GetId("OffensiveMagicSkill"),
                            name: _name,
                            description: _description,
                            cooldownTime: _cooldownTime,
                            damage: _damage,
                            ...
                            range: _range,
                            cost: _cost);
        var scriptableSkill = ScriptableObject.CreateInstance<OffensiveMagicSkillData>();
        scriptableSkill.Skill = skill;
        return scriptableSkill;
    }
